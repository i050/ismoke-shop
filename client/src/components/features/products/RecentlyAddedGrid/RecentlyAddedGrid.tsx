...
import styles from './RecentlyAddedGrid.module.css';
...
/**
 * קומפוננטת RecentlyAddedGrid - מציגה מוצרים שנוספו לאחרונה בגריד
 * תומכת בלחיצה על "טען עוד" לטעינה הדרגתית של מוצרים נוספים
 */
const RecentlyAddedGrid: React.FC<RecentlyAddedGridProps> = ({
  initialCount = 10,
  loadMoreCount = 10,
}) => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (errorObj?.status || errorObj?.name === 'ApiError' || err instanceof ApiError) {
          setError({ message, status: errorObj.status });
        } else {
